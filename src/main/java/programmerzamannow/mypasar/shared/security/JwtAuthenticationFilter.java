...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        // 🌟 BUNGKUS DENGAN TRY-CATCH BIAR TIDAK TERJADI EROR 500 MENTAH
        try {
            username = jwtService.extractUsername(token);
        } catch (io.jsonwebtoken.ExpiredJwtException e) {
            log.warn("SECURITY WARN - Token provided is expired: {}", e.getMessage());

            response.setStatus(HttpStatus.UNAUTHORIZED.value());
            response.setContentType("application/json");
            response.getWriter().write("{\"errors\": \"Unauthorized: Token has expired. Please login again.\"}");
            return; // Stop di sini, jangan lanjut!
        } catch (Exception e) {
            log.error("SECURITY ERROR - Token validation error: {}", e.getMessage());

            response.setStatus(HttpStatus.UNAUTHORIZED.value());
            response.setContentType("application/json");
            response.getWriter().write("{\"errors\": \"Unauthorized: Invalid token.\"}");
            return;
        }
        ...
        if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            ...
            log.info("Device ID from Header: {}", deviceId);
